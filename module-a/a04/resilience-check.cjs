...
try{
 ...
 const guide=await page.evaluate(()=>{const win=document.querySelector('iframe').contentWindow,doc=win.document,state=win.shuilongTemple.getA04State();return {state,labels:[...doc.querySelectorAll('.mural-label')].filter(el=>!el.hidden&&getComputedStyle(el).display!=='none').map(el=>({text:el.textContent,opacity:Number(el.style.opacity)})),routeLabels:[...doc.querySelectorAll('span')].filter(el=>el.textContent.match(/^(01 · 出发|02|03)$/)&&el.style.display==='block').length};});
 assert.equal(guide.state.nextGuideMuralId,'mural-05');assert.deepEqual(guide.state.camera,guide.state.overviewCamera);assert.ok(guide.state.bounds.left>0&&guide.state.bounds.right<1&&guide.state.bounds.bottom<.8);assert.ok(guide.state.routeOpacity>.7);assert.equal(guide.routeLabels,3);assert.ok(guide.labels.some(el=>el.text.includes('第五幅')&&el.opacity>.9));report.guideStart={bounds:guide.state.bounds,routeOpacity:guide.state.routeOpacity,routeLabels:guide.routeLabels,labels:guide.labels};
 await page.close();
 ...
 fs.writeFileSync(path.resolve(__dirname,'../../docs/validation/a04/resilience.json'),JSON.stringify(report,null,2));console.log('PASS: delayed model, failed model, resize, guide route and labels');
