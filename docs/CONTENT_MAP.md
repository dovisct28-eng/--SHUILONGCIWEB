...
- 用户补充确认：第三幅、第四幅与“出兵·入将”主题无关，只用于本段建立五幅位置总览；核心三幅强调后不再进入 A03—A08，也不需要提供第三、第四幅图片。
...
- 展示内容：从 A04 全景中的第五幅节点进入真实壁画。先给一次整体介绍，再以统一尺度从画面最右端连续巡视到最左端；末端提示 ⑤→①。没有局部释读、热点或缩放。
- 页面介绍草稿：这是本项目空间观看路线的起点——第五幅。刘灿姣、林伟（2016）记录第五铺损坏较重、人物难以统计，并将其解释为祭祀后离开的场景；后者是论文的研究观点，不是仅由残存画面证实的定论。接下来从右端向左端，完整经过这幅壁画。
- 信息类型：路线起点属于项目设计；损坏和人物统计状况为刘灿姣、林伟的记录；祭祀后离开属于该文释读。
- 图片：`水龙祠壁画素材/网页展示图/mural-05-display.webp`（6000×1617，Web 展示图）；`detail.webp` 不进入模块 A 基础导读。
- 来源：刘灿姣、林伟（2016），《湖南江永水龙祠壁画的发现报告》，PDF 第 3 页／刊物第 20 页；项目既定的第五幅→第一幅→第二幅数字观看路径。
- 核验状态：已读取上述论文原文并核对表述；图像的最终版本、裁切、原色对应及现场墙位仍待确认。论文解释不作为图像细节识别依据。

### A06 第一幅壁画导读

- 展示内容：⑤→① 简短提示后进入第一幅，先做一次整体介绍，再以同一组件从右端连续巡视到左端；末端提示 ①→②。第一幅是独立的完整观看章节。
- 页面介绍：沿本项目的观看路线，从第五幅来到第一幅。刘灿姣、林伟（2016）的发现报告记载，第一铺位于右面走廊，并描述了画面中骑马与步行的人物；这些是该文的现场记录，不据此推定人物身份或仪式名称。接下来从画面右端向左端，完整观看这一幅壁画。
- 图片：`水龙祠壁画素材/网页展示图/mural-01-display.webp`（2880×990）；`detail.webp` 不进入模块 A 基础导读。
- 来源：项目既定章节顺序；刘灿姣、林伟（2016），《湖南江永水龙祠壁画的发现报告》，刊物第 20 页／本地 PDF 第 3 页。
- 核验状态：已读取上述原文并核对位置与画面总体描述；不得凭残损图像指定人物、仪式或象征意义。

### A07 第二幅《入将图》导读

- 展示内容：①→② 简短提示后进入第二幅《入将图》，先做一次整体介绍，再以同一组件从右端连续巡视到左端。A07 可有稍长的介绍和滚动距离，局部细节探索归模块 B。
- 页面介绍：这是本项目三幅整体导读的最后一幅，也是后续《入将图》自主探索的对象。刘灿姣、林伟（2016）记录第二铺位于大殿右侧；对画中人物及仪式含义，胡彬彬、吴灿（2016）与李济民（2025）提出各自的解释，本页不将这些解释视为定论，也不展开局部释读。接下来从右端向左端完整观看。
- `researchNotes`：以下沿用既有研究记录，保留作内部资料，不等于 A07 页面正文或局部导读要求：
...
- 图片：`水龙祠壁画素材/网页展示图/mural-02-display.webp`（6000×1357）；`detail.webp` 不进入模块 A 基础导读。
- 来源：刘灿姣、林伟（2016），刊物第 20 页／本地 PDF 第 3 页；胡彬彬、吴灿（2016），刊物第 22 页／本地 PDF 第 1 页；李济民（2025），刊物第 116 页／本地 PDF 第 1 页。其它研究记录参见李济民 2025，pp.117–121；胡彬彬、吴灿 2016，pp.22–31；吴灿 2017，pp.119–125。
- 核验状态：已读取上屏所引原文；用户已确认第二幅对应模块 B《入将图》。研究记录中的人物身份、仪式和寓意不标为“已确认”。

### A08 《入将图》探索交接

- 展示内容：沿用 A07 完整《入将图》，收起导读标注，突出标题与“进入探索”按钮；不再讲解新线索。用户点击后才进入模块 B，滚动不自动跳转。

- 入口与模块关系：模块 B 大屏 WebGL 版 `http://localhost:3000/`；模块 A 只负责明确跳转和返回状态，不接管模块 B 内部交互。
- 来源：用户确认的交接目标与 [设计规格](DESIGN_SPEC.md)。
- 核验状态：入口目标已确认；A08 页面及联调待开发。第二幅项目编号与模块 B 目标图像对应已确认，但真实壁面方位仍待现场或权威资料核验。
