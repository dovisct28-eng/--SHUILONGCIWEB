...
模块 A 使用 `display.webp` 表现整体壁画。`detail.webp` 留给模块 B，模块 A 不加载 detail 资源。A05、A06、A07 均通过同一配置取得对应 display 图，并分别在章节接近时按需加载。
...
GLB 材质调色板原本由不同 mural 节点共享；运行时创建 mural 网格时克隆一次材质，使 A05 的 transfer opacity 不会影响第一至第四幅。材质克隆不发生在滚动帧中。
